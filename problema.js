...
// funcion de reporte
function crearEncabezadoReporte(type, from, to) {
  return `=== ${titulo} ===\nDesde: ${from}\nHasta: ${to}\n`;
}
function calcularEstadisticas(data, campo) {
  let total = 0;
  let max = 0;
  let min = 999999999;
...
function calcStats(values) {
  ...
  return { total, avg: total / (values.length || 1), max: Math.max(...values), min: Math.min(...values) };
}
